...
- `phraseforge-lang-<iso>` — per-language conventions (transcription, vocab shape, inflection, formality). Load the one matching the source language if it exists.
- `phraseforge-ebook` — render the lesson as `{start-*}`-fenced Markdown + `ebook.yml` for the cli-tools ebook builder (EPUB / A5 PDF / Docusaurus MDX). **The output target.**
...
- `tools/ff-parser.py` — parse a `*.ff` PhraseForge source file and emit a `Lesson` JSON object compatible with the output skills.
  ...
  **Pipe output to an export tool:**
  ...
  uv run --script tools/ff-parser.py source.ff --chunk 1 \
    | uv run --script ../phraseforge-ebook/tools/ebook-export.py --out chapter.md
  ```
