...
async function listFiles(dir, extension) {
  ...
  return entries
    .filter(
      e =>
        e.isFile() &&
        (!extension || e.name.endsWith(extension))
    )
    .map(e => e.name);
}
...
async function validateManifest() {
  ...
  if (!config.schemaVersion) {
    error(
      "agent-tools.yaml missing schemaVersion"
    );
  }

  if (!config.name) {
    error(
      "agent-tools.yaml missing name"
    );
  }
...
async function validateAgents() {
  ...
  for (const file of await listFiles(dir, ".md")) {
    try {
      ...
      if (!parsed.attributes.name) {
        warn(
          `Agent '${file}' missing name`
        );
      }

      if (!parsed.attributes.description) {
        warn(
          `Agent '${file}' missing description`
        );
      }
  ...
  for (const file of await listFiles(dir, ".md")) {
    ...
    for (const match of content.matchAll(
      ...
      if (
        ...
        warn(
          `Workflow '${file}' references unknown '${name}'`
        );
      }
    }

    ok(
      `Workflow validated: ${file}`
    );
  }
}
...
async function validateTargets(config) {
  ...
  for (const [name, target] of Object.entries(config.targets)) {
    ...
    if (!target.adapter) {
      ...
      error(
        `Adapter missing for '${name}': ${target.adapter}`
      );
    } else {
      ok(
        `Adapter found: ${name}`
    ...
    if (
      ...
      error(
        `Package directory missing for '${name}': ${packageDir}`
      );
      continue;
    }
    ...
    if (
      !(await exists(packageJson))
    ) {
      error(
        `Missing package.json: ${packageJson}`
      );
      continue;
    }
    ...
    if (pkg.name !== target.package) {
      error(
        `Package name mismatch: expected ${target.package}, found ${pkg.name}`
      );
    } else {
      ok(
        `Package validated: ${target.package}`
      );
    }
  }
}
...
async function main() {
  ...
  if (errors.length) {
    console.log(
      `${COLORS.red}Validation failed${COLORS.reset}: ${errors.length} error(s)`
    );
